...
class FormalHoldoutSnapshot(StrictEvidenceModel):
    declaration_manifest_sha256: Sha256 = Field(pattern=r"^[0-9a-f]{64}$")
    lock_start_receipt_sha256: Sha256 = Field(pattern=r"^[0-9a-f]{64}$")
    declared_harness_sha256: Sha256 = Field(pattern=r"^[0-9a-f]{64}$")
    regression_bundle_integrity_sha256: Sha256 = Field(pattern=r"^[0-9a-f]{64}$")
    regression_gate_sha256: Sha256 = Field(pattern=r"^[0-9a-f]{64}$")
    regression_run_id: str = Field(pattern=r"^[a-z0-9][a-z0-9._-]{7,79}$")
    regression_source_git_commit: str = Field(pattern=r"^[0-9a-f]{40,64}$")
...
class EvalSnapshot(StrictEvidenceModel):
    ...
    def keep_holdout_ids_withheld(self) -> EvalSnapshot:
        ...
        if self.split == "dev" and self.semantic_calibration is not None:
            raise ValueError("Development manifest cannot claim formal calibration")
        if self.split == "dev" and self.formal_holdout is not None:
            raise ValueError("Development manifest cannot claim a formal holdout")
        return self
    ...
    def validate_formal_contract(self) -> ReadonlyManifest:
        ...
        if self.purpose != "holdout_formal":
            if self.eval.split != "dev":
                raise ValueError("Only holdout_formal can use the holdout split")
            if (
                self.eval.semantic_calibration is not None
                or self.eval.formal_holdout is not None
            ):
                raise ValueError("Non-formal evidence cannot claim formal attestations")
            if self.purpose in {"diagnostic", "dev_repeat"}:
                ...
                if (
                    ...
                    or self.eval.case_set_sha256 != contract.case_set_sha256
                    or self.eval.case_ids != list(contract.case_ids)
                    or self.execution.planned_trials != contract.planned_trials
                    or self.execution.case_order != list(contract.case_ids)
                    or self.status != "completed"
                    or self.execution.completed_trials != contract.planned_trials
                ):
                    raise ValueError(
                        f"{self.purpose} evidence is not canonical and completed"
                    )
            if self.purpose == "dev_repeat":
                if (
                    self.budget.enforcement_mode != "persistent_sqlite"
                    or self.budget.run_status != "completed"
                    or self.model.observed_models != [self.model.requested_model]
                ):
                    raise ValueError(
                        "dev_repeat paid evidence is not canonical and completed"
                    )
            return self
        ...
        if self.schema_version == "1.0":
            if (
                ...
                raise ValueError("Legacy formal evidence is restricted to retired v1")
            return self
        ...
        if (
            ...
            raise ValueError("Formal v2 evidence is missing mandatory attestations")
        return self
...
class BudgetAttemptBucket(StrictEvidenceModel):
    ...
    def validate_count_type(cls, value: Any) -> Any:
        if type(value) is not int or value < 1:
            raise ValueError("budget attempt bucket count must be a positive integer")
        return value
    ...
    def validate_attempt_state(self) -> BudgetAttemptBucket:
        ...
        known = (
            Decimal(self.known_cost_cny) if self.known_cost_cny is not None else None
        )
        if reserved <= 0:
...
def _require_attempt_amounts(
    *,
    ...
    if (
        ...
        raise ValueError("budget totals differ from attempt bucket evidence")
    if cumulative:
        ...
        if cny_to_units(Decimal(amount.remaining_execution_cny)) != expected_remaining:
            raise ValueError("budget remaining amount differs from attempt evidence")
...
class BudgetSummary(StrictEvidenceModel):
    ...
    def validate_mode(self) -> BudgetSummary:
        ...
        if (
            self.run.hard_limit_cny != self.cumulative.hard_limit_cny
            or self.run.execution_limit_cny != self.cumulative.execution_limit_cny
        ...
        cumulative_committed = Decimal(self.cumulative.committed_cny)
        cumulative_settled = Decimal(self.cumulative.settled_cny)
        expected_remaining = max(
            Decimal("0"),
            Decimal(self.cumulative.execution_limit_cny) - cumulative_committed,
        )
        if (
            cumulative_committed < run_committed
            or cumulative_settled < run_settled
            or self.cumulative.attempt_count < self.run.attempt_count
            or self.cumulative.reserved_count < self.run.reserved_count
            or self.cumulative.uncertain_count < self.run.uncertain_count
            or Decimal(self.cumulative.remaining_execution_cny) != expected_remaining
        ):
            raise ValueError("Cumulative budget evidence is inconsistent")
        if self.attempt_evidence is not None:
            if any(
                bucket.reserved_cny != self.reservation_cny_per_attempt
            ...
            for bucket in self.attempt_evidence.cumulative:
                cumulative_buckets[_attempt_bucket_key(bucket)] += bucket.count
            if any(
                count > cumulative_buckets[key] for key, count in run_buckets.items()
            ):
                raise ValueError(
                    "Current-run attempt buckets are absent from cumulative evidence"
                )
        return self
...
class ModelCallRecord(StrictEvidenceModel):
    ...
    def validate_usage_token_types(
        ...
        if not isinstance(value, dict) or any(
            not isinstance(key, str) or type(token_count) is not int or token_count < 0
            for key, token_count in value.items()
        ):
            raise ValueError("usage must map strings to non-negative integers")
        return value
...
class BusinessStateRecord(StrictEvidenceModel):
    ...
    def validate_hashes(self) -> BusinessStateRecord:
        ...
        if self.changed is None:
            ...
            or (self.changed and self.before_sha256 == self.after_sha256)
            or (not self.changed and self.before_sha256 != self.after_sha256)
        ):
...
class ReadonlyCaseRecord(StrictEvidenceModel):
    ...
    def validate_scores(self) -> ReadonlyCaseRecord:
        ...
        expected_scores = {category: True for category in SCORE_CATEGORIES}
        ...
        expected_checks = [check.message for check in self.score_checks if check.passed]
        expected_failures = [
            check.message for check in self.score_checks if not check.passed
        ]
        if self.scores != expected_scores:
            raise ValueError("Case scores differ from their score checks")
        if self.checks != expected_checks or self.failures != expected_failures:
            raise ValueError("Case check and failure lists differ from score checks")
...
def _recompute_summary_without_budget(
    *,
    ...
    total = len(records)
    strict_passed = sum(record.status == "passed" for record in records)
    grouped: dict[str, list[ReadonlyCaseRecord]] = defaultdict(list)
    for record in records:
        grouped[record.case_id].append(record)
    cases_all_trials_passed = sum(
        len(case_records) == planned_trials
        and all(record.status == "passed" for record in case_records)
        for case_records in grouped.values()
    )
    score_layers: dict[str, dict[str, int | float]] = {}
    for category in SCORE_CATEGORIES:
        passed = sum(record.scores[category] for record in records)
    ...
    security_passed = sum(record.scores["security"] for record in records)
    changed_trials = sum(record.business_state.changed is True for record in records)
    unknown_trials = sum(record.business_state.changed is None for record in records)
    errors = Counter(
    ...
    return {
        ...
        "latency_ms": {
            "case": _summary_distribution([record.duration_ms for record in records]),
            "model_call": _summary_distribution(model_call_latencies),
        },
        "business_state": {
            "changed_trials": changed_trials,
            "unknown_trials": unknown_trials,
            "all_trials_unchanged": (changed_trials == 0 and unknown_trials == 0),
        },
...
def _require_completed_paid_bundle_records(
    *,
    ...
    for item in records:
        trials_by_case.setdefault(item.case_id, set()).add(item.trial)
    ...
    if (
        ...
        or any(trials != expected_trial_numbers for trials in trials_by_case.values())
        or (
            expected_case_ids is not None
            and set(trials_by_case) != set(expected_case_ids)
        )
        or summary.planned_trials != expected_trials
        or summary.total_trials != expected_case_count * expected_trials
    ):
        raise ValueError(f"{label} evidence is missing canonical case trials")
    ...
    if (
        ...
        raise ValueError(f"{label} budget is not completed and settled")
    try:
        ...
        require_canonical_attempt_reservation(
            ...
            max_output_tokens=(manifest.model.generation_config.max_tokens),
    ...
    for record in records:
        ...
        for call in calls:
            if call.started_at.tzinfo is None or not (
                record.started_at <= call.started_at <= record.completed_at
            ):
            ...
            if (
                ...
                raise ValueError(f"{label} model-call usage is not exact")
    ...
    try:
        ...
        raise ValueError(f"{label} usage cannot be priced") from exc
    ...
    for bucket in attempt_evidence.run:
        if (
            ...
            raise ValueError(f"{label} current-run attempt is unsettled")
        actual_buckets[
            (
                bucket.status,
                bucket.settlement_mode,
    ...
    cumulative_committed = Decimal(cumulative_budget.committed_cny)
    expected_remaining = max(
        Decimal("0"),
        Decimal(cumulative_budget.execution_limit_cny) - cumulative_committed,
    )
    if (
...
class ReadonlyEvidenceBundle(StrictEvidenceModel):
    ...
    def cross_validate(self) -> ReadonlyEvidenceBundle:
        ...
        if (
            ...
            raise ValueError("Manifest and summary budget reservations differ")
        if self.manifest.budget.run_status != self.summary.budget.run_status:
        ...
        if self.manifest.budget.enforcement_mode == "persistent_sqlite":
            if (
                ...
                raise ValueError("Manifest and persisted budget run identity differ")
        ...
        if self.manifest.budget.enforcement_mode == "persistent_sqlite":
            if (
                ...
                or self.manifest.budget.price_source_url != summary_price.source_url
                or self.manifest.budget.usage_source_url
                != summary_price.usage_source_url
                or self.manifest.budget.price_captured_at != summary_price.captured_at
                or self.manifest.budget.price_valid_until != summary_price.valid_until
                or summary_price.model != self.manifest.model.requested_model
            ):
                raise ValueError("Manifest and summary pricing evidence differ")
        ...
        cases_by_key = {(item.case_id, item.trial): item for item in self.cases}
        trajectories_by_key = {
        ...
        if (
            ...
            model_calls = [call for item in self.cases for call in item.model_calls]
            run_budget = self.summary.budget.run
            cumulative_budget = self.summary.budget.cumulative
            if summary_price is None:
                raise ValueError("Formal v2 evidence requires exact pricing")
            try:
                canonical_price = require_canonical_paid_budget(
                    price=summary_price,
                    expected_model=(self.manifest.model.requested_model),
                    run_hard_limit_cny=(run_budget.hard_limit_cny),
                    run_execution_limit_cny=(run_budget.execution_limit_cny),
                    cumulative_hard_limit_cny=(cumulative_budget.hard_limit_cny),
            ...
            if (
                self.manifest.harness.canonical_price_snapshot_sha256
                != canonical_price_file_sha256()
                or self.manifest.created_at < canonical_price.captured_at
                or self.manifest.completed_at > canonical_price.valid_until
            ):
                raise ValueError(
            ...
            if any(
                ...
                raise ValueError("Formal v2 evidence requires exact call usage")
            try:
            ...
            if (
                ...
                or any(trials != {1, 2, 3, 4} for trials in trials_by_case.values())
                or self.summary.planned_trials != 4
                or self.summary.total_trials != 80
                or observed_models != {self.manifest.model.requested_model}
                or run_budget.attempt_count != provider_attempts
                or run_budget.reserved_count != 0
                or run_budget.uncertain_count != 0
                or cumulative_budget.reserved_count != 0
                ...
                or cumulative_committed > Decimal(cumulative_budget.execution_limit_cny)
                or Decimal(cumulative_budget.remaining_execution_cny)
                != expected_remaining
            ):
                raise ValueError("Formal v2 evidence failed cross-validation")
        return self
