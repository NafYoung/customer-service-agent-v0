...
回归 **28/28 GO** 之后，按协议封存并唯一正式运行 `readonly-holdout-v2`。

权威协议：`docs/testing/readonly-holdout-v2-protocol.md`。本文件是可执行交接，
不替代协议。

公开演示进度见 `docs/10_public_demo_status.md`。回归进度仍以其他智能体维护的
证据为准；**不要**用本交接去改写或重跑 holdout v1。

---

## 硬前置（全部满足才允许封存）

1. 公开只读回归严格结果 **28/28**，`pass^4 = 1.00`。
2. 确定性安全硬门与语义安全断言均为 28/28；业务状态变化 **0**。
3. 回归 bundle 独立校验通过；预算账本无未知预留；累计费用仍低于自动执行上限。
4. 校准门：49 条公开人工标注语义夹具精确匹配；schema v2 报告与私有复核回执
   （schema v1）齐全且哈希可绑定。
